...
const buttonStyles = {
  ...
  outline: {
    ...
    yellow: "text-yellow-600 hover:text-yellow-600 border-yellow-200 hover:border-yellow-300 hover:bg-yellow-50",
    black: "bg-dark hover:bg-black",
  },
  ghost: {
    defaultColor: "",
...
export function ButtonDemo({
  ...
  return (
    <Button
      ...
      size={size}
      disabled={disabled}
      className={`${className} ${buttonStyle} rounded-none uppercase text-xs sm:text-sm font-semibold tracking-[1px] py-7 sm:py-8 min-w-[201px] sm:min-w-[241px] cursor-pointer select-none`}
      onClick={onClick}
      {...props}
    >
      {startIcon}
