...
export function SelectScrollable({
  ...
  return (
    <Select
      ...
      // open={true}
    >
      <SelectTrigger className={`select-scrollable-trigger w-[280px] cursor-pointer rounded-none border-line hover:border-[rgba(0,0,0,0.2)] ${triggerClassName}`}>
      ...
      <SelectContent className={`select-scrollable-content max-w-[95vw]  ${contentClassName}`}>
        {items.length ? (
          items.map((item, index) => {
            return (
              <SelectItem key={index} value={item.value.toString()} className="select-scrollable-item cursor-pointer">
